...
from utils import get_model_params, Baseline
import numpy as np
import flwr as fl
...
def weighted_average(metrics):
    ...
    c_weights = [num_examples * m["test_c_index"] for num_examples, m in metrics]

    loss_weights = [num_examples * m["test_loss"] for num_examples, m in metrics]
    cid = [m["cid"] for num_examples, m in metrics]

    examples = [num_examples for num_examples, _ in metrics]

    total_examples = sum(examples)
    test_c_index = sum(c_weights) / total_examples

    test_loss = sum(loss_weights) / total_examples

    print(
        f"This should be Client ID joined for the evaluation {cid}  || Aggregated C-index: {test_c_index} || Aggregated Loss: {test_loss}")

    wandb.log({"Aggregated c_index": test_c_index,"Aggregated loss": test_loss, "cid": cid})

    return {"Test c_index": test_c_index, "test_loss": test_loss}
...
def main():
    ...
    strategy = SaveModelStrategy(
        ...
        #initial_parameters=fl.common.ndarrays_to_parameters(get_model_params(Baseline())),
